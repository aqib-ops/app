...
export function FeaturedProjects() {
  ...
  const hasContent = Boolean(featuredProjectsConfig.titleRegular) || featuredProjectsConfig.projects.length > 0;
        ...
        <div ref={headerRef} className="flex flex-col md:flex-row md:items-end md:justify-between mb-16 md:mb-20 opacity-0">
          ...
          {featuredProjectsConfig.viewAllText && (
            <a
              ...
              className="mt-6 md:mt-0 inline-flex items-center gap-2 text-white/70 hover:text-white text-sm transition-colors duration-300 group"
            >
              {featuredProjectsConfig.viewAllText}
          ...
          {featuredProjectsConfig.projects.map((project, index) => (
            ...
            >
              ...
              >
                ...
                {/* Viewfinder overlay with neon green */}
                <div className="absolute inset-0 pointer-events-none">
                  <div 
                    ...
                    style={{ borderColor: 'rgba(57, 255, 20, 0.7)' }}
                  />
                  <div 
                  ...
                  <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    ...
                    <div className="w-6 h-px -mt-3 -ml-[11px] bg-white/50" />
                  </div>
                </div>
              ...
              <div className={`project-content opacity-0 ${index % 2 === 1 ? 'md:order-1 md:text-right' : ''}`}>
                ...
                <h3 className="project-text-item text-2xl md:text-3xl lg:text-4xl font-bold text-white tracking-tight mb-4">
                  {project.title}
                </h3>
                <p className="project-text-item text-white/60 text-base md:text-lg leading-relaxed mb-6">
                  {project.description}
                </p>
                {featuredProjectsConfig.viewProjectText && (
                  <a
                    href="#contact"
                    className={`project-text-item inline-flex items-center gap-2 text-white text-sm border-b pb-1 hover:border-white transition-colors duration-300 group/link ${
                      index % 2 === 1 ? 'md:flex-row-reverse' : ''
                    }`}
